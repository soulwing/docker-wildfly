...
This is a Docker image for Wildfly. It is based on the Alpine variant of the official [OpenJDK container image](https://hub.docker.com/_/openjdk/) and uses @just-containers [s6-overlay](https://github.com/just-containers/s6-overlay) to run Wildfly.

## Build

```
docker image build -t soulwing/wildfly .
```
...
This specifies the image tag for the OpenJDK image to use as the base image.
The default is specified in the Dockerfile.

### `WILDFLY_VERSION` 

This identifies the version of Wildfly to build into the container image.
The default is specified in the Dockerfile.

### `S6_VERSION` 

This identifies the version of s6-overlay  to build into the container image.
The default is specified in the Dockerfile.
